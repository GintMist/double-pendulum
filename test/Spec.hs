...
instance Arbitrary DoublePendulumSystem where
    arbitrary = do
        ...
        o1 <- arb
        o2 <- arb        
        return (DPS (Pendulum m1 l1 t1 o1) (Pendulum m2 l2 t2 o2) g 0 0.005)
        where
            arb :: Gen Float
            arb = do
                x <- arbitrary
...
mechEnergy (DPS (Pendulum m1 l1 t1 o1) (Pendulum m2 l2 t2 o2) g _ _) = - (m1 + m2) * g * l1 * cos t1 - m2 * g * l2 * cos t2  
                                                                     + (1/2) * m1 * l1 * l1 * o1 * o1
