...
derivatives (DPS (Pendulum m1 l1 theta1 omega1 _ _) (Pendulum m2 l2 theta2 omega2 _ _) gc _) = [theta1', omega1', theta2', omega2']
  where
    theta1' = omega1
    theta2' = omega2
    delta = theta2 - theta1
    ...
    omega1' = (m2 * l1 * omega1 * omega1 * sin delta * cos delta
              + m2 * gc * sin theta2 * cos delta
              + m2 * l2 * omega2 * omega2 * sin delta
              - totalMass * gc * sin theta1)
              / den1
    omega2' = (- m2 * l2 * omega2 * omega2 * sin delta * cos delta
              + totalMass * gc * sin theta1 * cos delta
              - totalMass * l1 * omega1 * omega1 * sin delta
              - totalMass * gc * sin theta2)
              / den2

rk4PendulumSolver ps = mkps yo
  where
    (DPS (Pendulum m1 l1 theta1 omega1 (ox1, oy1) p1) (Pendulum m2 l2 theta2 omega2 (ox2, oy2) p2) gc cdt) = ps
    mkps [a,b,c,d] = DPS (Pendulum m1 l1 a b (nx1, ny1) np1) (Pendulum m2 l2 c d (nx2, ny2) np2) gc cdt
    yi = [theta1, omega1, theta2, omega2]
    k1 = map (* cdt) (derivatives ps)
    k2 = map (* cdt) (derivatives (mkps $ zipWith (+) yi (map (* 0.5) k1)))
    k3 = map (* cdt) (derivatives (mkps $ zipWith (+) yi (map (* 0.5) k2)))
    k4 = map (* cdt) (derivatives (mkps $ zipWith (+) yi k3))
    yo = zipWith (+) yi
        (zipWith (+) (map (/ 6) k1)
        (zipWith (+) (map (/ 3) k2)
...
dpsdrawer (DPS (Pendulum m1 _ _ _ (a, b) p1) (Pendulum m2 _ _ _ (c, d) p2) _ _) =
...
anim :: [Float] -> IO ()
anim [m1, l1, t1, o1, m2, l2, t2, o2, gc] =
  simulate (InWindow "DPS" (ceiling $ (l1 + l2) * 220, ceiling $ (l1 + l2) * 220) (10, 10))
           black
           200
           (DPS (Pendulum m1 l1 t1 o1 (nx1,ny1) []) (Pendulum m2 l2 t2 o2 (nx2,ny2) []) gc (1/200))
           dpsdrawer
